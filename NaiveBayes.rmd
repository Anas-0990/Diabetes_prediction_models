---
title: " Naive Bayes Model For Diabetics prediction"
author: "Anas"
date: "22/08/2024"
...
#  Naive Bayes - Training using cross validation approach
```{r Naive_Bayes, echo=FALSE}
train_x1 <- as.matrix(train_x)


library(naivebayes)
NB = naive_bayes(train_x1, train_y)
...
# Perform predictions for the test data. Type can be class or prob for NB.
pred = predict(NB, newdata = test_x1, type = "class")

library(caret)
NB_comf_mat <- confusionMatrix(data = test_y, reference = pred, positive = "1")
NB_comf_mat
...
# Work for Brier Score - IM-BALANCED dataset
```{r}
pred_DB = predict(NB, newdata = test_x1, type = "prob")
...
# SMOTE on the training data and the NB model on it. 
```{r}
...
NB_SMOTE = naive_bayes(xtrain_smote1, ytrain_smote)
...
# Perform predictions for the test data. Type can be class or prob for NB.
pred_DB = predict(NB_SMOTE, newdata = xtest_smote1, type = "class")

# Convert to vectors.
pred_DB <- as.factor(pred_DB)
ytest_smote <- as.factor(ytest_smote)

# Same level.
levels(pred_DB) <- levels(ytest_smote)

# Confusion Matrix
library(caret)
NB_conf_mat_SMOTE <- confusionMatrix(data = pred_DB, reference = ytest_smote, positive = "1")
NB_conf_mat_SMOTE
...
```{r}
pred_DB = predict(NB_SMOTE, newdata = xtest_smote1, type = "prob")
